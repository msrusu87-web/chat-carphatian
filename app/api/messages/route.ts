...
import { messages, users, profiles } from '@/lib/db/schema'
...
export async function POST(req: NextRequest) {
    try {
        ...
        const senderName = senderProfile?.full_name || session.user.email?.split('@')[0] || 'Someone'
        ...
        // Note: In a real app, you'd want to debounce this to avoid spamming
        // e.g., only send if recipient hasn't been online in 5 minutes
        sendNewMessageEmail(
            recipientId,
            senderName,
            content,
            currentUser.id // Use sender ID as conversation reference
